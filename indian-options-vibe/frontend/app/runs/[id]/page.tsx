...
export default function RunDetailPage({ params }: { params: { id: string } }) {
  ...
  const [source, setSource] = useState<'saved' | 'mock' | 'missing'>('missing');
  ...
  if (!run) {
    return (
      <section className="p-8 md:p-12">
        <div className="mx-auto max-w-5xl rounded-3xl border border-red-900 bg-red-950/20 p-6">
          <h1 className="text-2xl font-bold text-white">Run not found</h1>
          <p className="mt-2 text-red-200">This run is not in local history or mock runs.</p>
          <Link href="/runs" className="mt-5 inline-block rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-slate-950">Back to Runs</Link>
        </div>
      </section>
    );
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
          ...
          <div className="rounded-2xl border border-emerald-900/70 bg-emerald-950/20 px-4 py-3 text-sm text-emerald-200">
            {run.status} • {source === 'saved' ? 'Saved run' : 'Mock run'} • Live locked
          </div>
        </div>

        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <MetricCard label="Net P&L" value={`₹${run.netPnl.toLocaleString('en-IN')}`} hint="after estimated charges" />
          <MetricCard label="Win Rate" value={`${run.winRate}%`} hint={`${run.totalTrades} trades`} />
          <MetricCard label="Profit Factor" value={`${run.profitFactor}`} hint="gross wins / gross losses" />
          <MetricCard label="Max Drawdown" value={`₹${run.maxDrawdown.toLocaleString('en-IN')}`} hint="worst dip" />
        </div>

        <div className="mt-4 grid gap-4 md:grid-cols-4">
          <MetricCard label="Charges" value={`₹${(run.charges || 0).toLocaleString('en-IN')}`} hint="brokerage + taxes mock" />
          <MetricCard label="Avg Win" value={`₹${avgWin.toLocaleString('en-IN')}`} />
          <MetricCard label="Avg Loss" value={`₹${avgLoss.toLocaleString('en-IN')}`} />
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 lg:col-span-2">
            <div className="flex flex-col justify-between gap-2 md:flex-row md:items-end">
              <div><h2 className="text-xl font-bold text-white">Equity Curve Placeholder</h2><p className="mt-1 text-sm text-slate-400">Run curve placeholder. Real chart will come from persisted backend trade results later.</p></div>
            ...
            <div className="mt-5 flex h-72 items-end gap-3 rounded-2xl border border-slate-800 bg-slate-950 p-5">
              {equityPoints.map((point, index) => <div key={`${point}-${index}`} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-xl bg-emerald-500/30" style={{ height: `${28 + index * 7 + (index % 3) * 18}px` }} /><span className="text-[10px] text-slate-500">{point}</span></div>)}
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-300">
              <RiskLine status="pass" text="Paper mode only" />
              <RiskLine status="pass" text="Live orders locked" />
              <RiskLine status={run.maxDrawdown <= -10000 ? 'warn' : 'pass'} text={run.maxDrawdown <= -10000 ? 'Drawdown needs review' : 'Drawdown within review threshold'} />
              <RiskLine status="warn" text="Backtest uses MVP dummy data until real feed is connected" />
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-3xl border border-slate-800 bg-slate-900/70 p-5">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
            <div><h2 className="text-xl font-bold text-white">Trade Table</h2><p className="mt-1 text-sm text-slate-400">Trades saved with this run or sample trades for mock reports.</p></div>
            <DownloadCsvButton filename={`${run.id}-trades.csv`} rows={csvRows} />
          </div>
          <div className="mt-5 overflow-hidden rounded-2xl border border-slate-800">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-950 text-slate-400"><tr>{['Time', 'Contract', 'Side', 'Entry', 'Exit', 'P&L', 'Result'].map((h) => <th key={h} className="p-3 font-medium">{h}</th>)}</tr></thead>
              <tbody>{trades.map((trade, index) => <tr key={`${trade.time}-${trade.symbol}-${index}`} className="border-t border-slate-800 text-slate-300"><td className="p-3">{trade.time}</td><td className="p-3 font-medium text-white">{trade.symbol}</td><td className="p-3">{trade.side}</td><td className="p-3">{trade.entry}</td><td className="p-3">{trade.exit}</td><td className={trade.pnl >= 0 ? 'p-3 text-emerald-300' : 'p-3 text-red-300'}>{trade.pnl >= 0 ? '+' : ''}₹{trade.pnl.toLocaleString('en-IN')}</td><td className="p-3">{trade.result}</td></tr>)}</tbody>
            </table>
          </div>
        </div>

        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5"><h2 className="text-xl font-bold text-white">Strategy Assumptions</h2><ul className="mt-4 space-y-3 text-sm text-slate-300">{assumptions.map((item) => <li key={item}>• {item}</li>)}</ul></div>
          <div className="rounded-3xl border border-slate-800 bg-slate-950 p-5"><h2 className="text-xl font-bold text-white">Generated Strategy Logic</h2><pre className="mt-4 overflow-x-auto rounded-2xl bg-black p-4 text-xs text-emerald-200">{`prompt = ${JSON.stringify(run.prompt || run.summary)}
contract = select_atm_option(symbol='${run.symbol}')
run_paper_backtest(contract, timeframe='${run.timeframe}')
keep_live_orders_locked()`}</pre></div>
        </div>
      </div>
    </section>
  );
