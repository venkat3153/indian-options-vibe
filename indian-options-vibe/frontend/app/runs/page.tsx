...
export default function RunsPage() {
  ...
  useEffect(() => {
    const saved = JSON.parse(window.localStorage.getItem('backtestRuns') || '[]') as SavedRun[];
    setSavedRuns(saved);
  }, []);

  const runs = useMemo(() => {
    const savedIds = new Set(savedRuns.map((run) => run.id));
    return [...savedRuns, ...mockRuns.filter((run) => !savedIds.has(run.id))];
  }, [savedRuns]);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-slate-400">Agent backtests are saved locally in this browser. Mock runs remain below until backend database persistence is added.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            {savedRuns.length > 0 ? <button onClick={clearSavedRuns} className="rounded-xl border border-red-900 px-4 py-3 text-sm text-red-300 hover:bg-red-950/30">Clear Saved Runs</button> : null}
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-5">
          <SummaryCard label="Total Runs" value={`${runs.length}`} hint={`${savedRuns.length} saved locally`} />
          ...
          <SummaryCard label="Total P&L" value={`₹${totalPnl.toLocaleString('en-IN')}`} hint="Saved + mock runs" tone={totalPnl >= 0 ? 'win' : 'loss'} />
