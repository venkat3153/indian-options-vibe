...
export default function StockDetailPage({ params }: { params: { symbol: string } }) {
  ...
  return <section className="p-8 md:p-12"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900/70 p-6"><div className="flex flex-col justify-between gap-3 md:flex-row md:items-start"><div><h2 className="text-2xl font-bold text-white">Research Breakdown</h2><p className="mt-1 text-sm text-slate-400">Transparent score components. This explains why the model is interested or cautious.</p></div><div className="rounded-2xl border border-emerald-800 bg-emerald-500/10 px-5 py-3 text-center"><div className="text-xs uppercase tracking-[0.18em] text-emerald-300">Final Decision</div><div className="mt-1 text-lg font-bold text-white">{signal?.label || 'Wait'}</div></div></div><div className="mt-5 grid gap-4 md:grid-cols-5">{breakdown.map((row) => <BreakdownCard key={row.label} row={row} />)}</div></div>

    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.8fr]"><div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6"><div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-bold text-white">Live Decision</h2><p className="mt-1 text-sm text-slate-400">This is a research decision, not automatic execution.</p></div>{signal ? <SignalBadge signal={signal} /> : null}</div><p className="mt-5 rounded-2xl border border-slate-800 bg-slate-950 p-5 text-sm leading-6 text-slate-300">{signal?.reason}</p><div className="mt-5 grid gap-3 md:grid-cols-3"><Box title="Entry Idea" text={getEntryIdea(stock)} /><Box title="Invalidation" text={getInvalidation(stock)} tone="loss" /><Box title="Target Idea" text={getTargetIdea(stock)} tone="win" /></div></div><div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6"><h2 className="text-2xl font-bold text-white">Buyer / Seller Zones</h2><p className="mt-1 text-sm text-slate-400">Estimated zones from current price and research levels. Volume profile and order book depth come later.</p><div className="mt-5 grid gap-3"><Zone title="Buyer zone" text={getBuyerZone(stock, quote)} tone="win" /><Zone title="Seller / supply zone" text={getSellerZone(stock, quote)} tone="loss" /><Zone title="No-trade warning" text={getNoTradeWarning(stock, quote)} /></div></div></div>
    ...
    <div className="mt-6 rounded-3xl border border-yellow-900 bg-yellow-950/20 p-5 text-sm text-yellow-100"><span className="font-bold">Safety rule:</span> Even if Live Strength is high, this page does not mean buy now. Use it to prepare a plan, then wait for intraday confirmation and daily discipline rules.</div>
  </div></section>;
}
