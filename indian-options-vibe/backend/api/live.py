...
def get_latest_close_by_symbol(limit: int = 5000) -> dict[str, float]:
    ...
    for candle in candles:
        ...
        candle_date = str(candle.get("candle_date", ""))
        close = float(candle.get("close") or 0)
        if symbol and close and (symbol not in latest or candle_date > latest[symbol][0]):
            latest[symbol] = (candle_date, close)
...
async def live_quotes(limit: int = 50) -> dict[str, Any]:
    ...
    latest_close = get_latest_close_by_symbol()
    ...
    for security_id, item in symbol_by_id.items():
        ...
        symbol = item["symbol"]
        prev_close = latest_close.get(symbol)
        change_pct = None
        change_value = None
    ...
    payload = {
        ...
        "quotes": quotes,
        "live_orders_enabled": False,
        "note": "Real-time snapshot from Dhan marketfeed LTP. This is polling, not WebSocket streaming yet.",
    }
